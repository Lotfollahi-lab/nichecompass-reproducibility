...
def generate_gene_programs(gene_orthologs_mapping_file_path, mebocost_enzyme_sensor_interactions_folder_path, config):
    ...
    if "collectri" in config["sources"]:
...
os.makedirs("processed-gene-programs:collectri", exist_ok=True)

file_name = "gene_programs.pkl"

local_file = os.path.join("processed-gene-programs:collectri", file_name)
